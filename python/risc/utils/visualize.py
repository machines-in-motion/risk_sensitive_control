...
class ConsimVisual(object):
    def __init__(self, name, filename, package_dirs, root_joint, consimVisualizer, visualOptions,):
        ...
        #         for i in range(4):
        #             frame_name = conf.point_frames[4*j + i]
    ...
    def removeObject(self, name):
        self.gui.deleteNode(self.RootNodeName+"/"+name, True)
    ...
    def add_box(self, name, position, size):
        ...
        self.gui.addBox(self.RootNodeName+"/"+name, size[0], size[1], size[2], color)
        self.gui.setVisibility(self.RootNodeName+"/"+name,"ON")
        self.gui.applyConfiguration(self.RootNodeName+"/"+name, pin.SE3ToXYZQUATtuple(obj_pose))
        self.gui.refresh()
    ...
    def displayContact(self, name, force, visibility="ON"):
        if visibility =="OFF":
            ...
            try:
                ...
                self.gui.setVisibility(coneName, "ALWAYS_ON_TOP")
            except:
